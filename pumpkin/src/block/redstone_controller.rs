...
use super::blocks::redstone_wire::RedstoneWireBlock;

trait RedstoneController {
    async fn update(
        server: &Server,
        world: &World,
        block_pos: &BlockPos,
        block: &Block,
        state: &BlockState,
        wire_orientation: Option<BlockDirection>,
        block_added: bool,
    );

    async fn get_strong_power_at(world: &World, block_pos: &BlockPos) -> u8 {
        RedstoneWireBlock::get_strong_power(world, block_pos).await
    }
    ...
    async fn calculate_wire_power_at(world: &World, block_pos: &BlockPos) -> u8 {
        ...
        for direction in BlockDirection::horizontal() {
            ...
            if other_block_state.is_solid && !up_block_state.is_solid {
                ...
                power = std::cmp::max(
                    power,
                    Self::get_wire_power_at(world, &other_up_pos).await as i8,
                );
            } else if !other_block_state.is_solid {
                let other_down_pos = other_pos.down();
                power = std::cmp::max(
                    power,
                    Self::get_wire_power_at(world, &other_down_pos).await as i8,
                );
            }
        }

        return std::cmp::max(power - 1, 0) as u8;
    }
}
...
impl RedstoneController for DefaultRedstoneController {
    async fn update(
        ...
        let power = Self::calculate_total_power_at(world, block_pos).await;
        ...
        if wire_props.power.to_index() as u8 != power {
            if world.get_block_state(block_pos).await.unwrap().id == state.id {
                ...
                world.set_block_state(block_pos, wire_props.to_state_id(&block)).await;
            }
...
impl DefaultRedstoneController {
    async fn calculate_total_power_at(world: &World, block_pos: &BlockPos) -> u8 {
        let power = Self::get_strong_power_at(world, block_pos).await;

        if power == 15 {
            return power;
        }

        std::cmp::max(power, Self::calculate_wire_power_at(world, block_pos).await)
    }
}
